...
export default {
	async fetch(request, env, ctx) {
		...
		const host = hosts[0];
		if (url.pathname.startsWith(`${本地后台资源前缀}/vendor/`)) return await 返回本地静态资源(env, request);
		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			if (!管理员密码) return await 返回本地后台页面(env, request, `${本地后台资源前缀}/noADMIN.html`, 404);
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					return await 返回本地后台页面(env, request, `${本地后台资源前缀}/login.html`);
					...
					return await 返回本地后台页面(env, request, `${本地后台资源前缀}/admin.html`);
			...
			} else if (!envUUID) return await 返回本地后台页面(env, request, `${本地后台资源前缀}/noKV.html`, 404);
		}
...
async function 返回本地静态资源(env, request) {
	if (!env.ASSETS || typeof env.ASSETS.fetch !== 'function') {
		return new Response('本地静态资源未绑定，请检查 wrangler.toml 的 ASSETS 配置。', {
			status: 500,
...
async function 返回本地后台页面(env, request, pathname, status = 200) {
	if (!env.ASSETS || typeof env.ASSETS.fetch !== 'function') {
		return new Response('本地后台页面未绑定，请使用 wrangler 部署并启用 ASSETS 绑定。', {
			status: 500,
			headers: { 'Content-Type': 'text/plain;charset=utf-8' }
		});
	}
	...
	if (!response.ok) return new Response('本地后台页面资源缺失。', { status: 500, headers });
